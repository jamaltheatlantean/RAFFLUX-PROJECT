...
contract Rafflux is VRFConsumerBaseV2, Ownable {
	...
	// the id is a keccak256 hash from the timestamp and sender address
	...
	// Keep track of the user tickts by id
	// the id is a keccak256 hash from the raffleId and ticketId
	...
	event TicketSold(uint256 raffleId, uint ticketId, address buyer);	
	// Emit an event when no more tickets left for a raffle
	event SoldOut(uint256 raffleId, uint256 requestId);
	// Emit an event when no more time left for a raffle
	event TimeOut(uint256 raffleId, uint256 requestId);
	...
	function _finishRaffle(uint256 _raffleId, uint256 _random) public {
